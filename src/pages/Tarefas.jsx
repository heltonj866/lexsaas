...
export default function Tarefas() {
  ...
  async function carregarDados() {
    ...
    } catch (error) { toast.error("Erro ao carregar os prazos."); } finally { setLoading(false); }
  }
  ...
  function prepararEdicao(tarefa) {
    ...
    setForm({
      ...
      status: tarefa.status || 'pendente', prioridade: tarefa.prioridade || 'media', processo_id: tarefa.processo_id || ''
    });
    setIsModalOpen(true);
  }
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-hidden pb-4 lg:pb-0">
          {colunas.map(coluna => (
            <div key={coluna.id} className={`flex flex-col rounded-2xl border ${coluna.cor} p-4 sm:p-5 transition-colors duration-300`} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, coluna.id)}>
              <h2 className="font-bold mb-4 flex justify-between items-center text-sm sm:text-base">
                {coluna.titulo}
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-1 custom-scrollbar max-h-[400px] lg:max-h-none">
                {tarefas.filter(t => t.status === coluna.id).map(tarefa => (
                  <div key={tarefa.id} draggable onDragStart={(e) => handleDragStart(e, tarefa.id)} className="bg-white dark:bg-slate-800 p-4 sm:p-5 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 cursor-move hover:shadow-md hover:border-slate-300 transition-all group animate-in zoom-in-95">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-slate-500 mt-3 pt-3 border-t border-slate-100 dark:border-slate-700">
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-slate-900 border dark:border-slate-800 shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="overflow-y-auto custom-scrollbar flex-1 p-5 sm:p-6">
              <form id="form-tar" onSubmit={handleSalvar} className="space-y-5 pb-8 px-1">
                ...
                <div className="bg-slate-50 dark:bg-slate-800/30 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 mt-2">
                  <label className={labelEstilo}>Vincular a um Processo (Opcional)</label>
