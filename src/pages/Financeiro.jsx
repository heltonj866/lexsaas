...
export default function Financeiro() {
  ...
  async function alternarStatus(lancamento) {
    ...
    try { await api.put(`/financeiro/${lancamento.id}`, { ...lancamento, status: novoStatus }); carregarDados(); } catch (error) { toast.error("Erro."); }
  }
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left text-sm whitespace-nowrap min-w-[700px]">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80">
              {lancamentos.map((l) => (
                <tr key={l.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 group">
                  <td className="px-4 sm:px-6 py-3 sm:py-4"><div className="font-bold text-slate-800 dark:text-slate-200 text-sm">{l.descricao}</div><div className="flex items-center gap-1 text-[10px] text-slate-500 mt-1">{l.processo?.numero_processo ? <><Briefcase size={12}/> {l.processo.numero_processo}</> : ''}{!l.processo?.numero_processo && l.cliente?.nome ? <><Users size={12}/> {l.cliente.nome}</> : ''}</div></td>
