...
export default function Clientes() {
  ...
  // 👇 MODO APRESENTAÇÃO: CARREGANDO DADOS FICTÍCIOS 👇
  async function carregarClientes(page = 1) {
    try {
      ...
      // Funcionalidade de busca a funcionar com os dados falsos
      if (busca) {
        dadosMock = dadosMock.filter(c => 
  ...
  // A BUSCA DE CEP CONTINUA REAL POIS USA A API PÚBLICA DO VIACEP
  ...
  async function handleSalvarCliente(e) {
    e.preventDefault();
    try {
      await new Promise(resolve => setTimeout(resolve, 600)); // Tempo de carregamento visual
      if (editandoId) {
  ...
  // 👇 MODO APRESENTAÇÃO: SIMULA EXCLUIR 👇
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="rounded-lg border border-slate-300 pl-10 pr-4 py-2.5 w-full sm:w-64 focus:ring-2 focus:ring-sky-500 outline-none transition-all"
              value={busca} onChange={e => setBusca(e.target.value)}
            />
          </div>
          <button 
            onClick={abrirNovo}
            className="flex items-center gap-2 bg-sky-600 text-white px-5 py-2.5 rounded-lg font-medium hover:bg-sky-700 transition-colors shadow-sm"
          >
            <Plus size={20} /> <span className="hidden sm:inline">Novo Cliente</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col flex-1 overflow-hidden">
                ...
                clientes.map((cliente) => (
                  <tr key={cliente.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-9 w-9 rounded-full bg-sky-100 text-sky-600 flex items-center justify-center font-bold">
                          {cliente.nome.charAt(0)}
                        </div>
                        <span className="font-bold text-slate-800">{cliente.nome}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-slate-500 font-mono text-xs">{cliente.cpf_cnpj || '-'}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        {/* Apenas no modo apresentação, o Eye redireciona para um id genérico */}
                        <Link to={`/clientes/${cliente.id}`} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Ver Ficha Completa">
                          <Eye size={18} />
                        </Link>
                        <button onClick={() => abrirEdicao(cliente)} className="p-2 text-slate-400 hover:text-sky-600 hover:bg-sky-50 rounded-lg transition-colors" title="Editar">
                          <Edit size={18} />
                        </button>
                        <button onClick={() => setClienteParaExcluir(cliente)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Excluir">
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </td>
        ...
        <div className="bg-slate-50 border-t border-slate-200 px-6 py-4 flex items-center justify-between">
          <p className="text-sm text-slate-500">
            A mostrar página <span className="font-bold text-slate-700">{paginaAtual}</span> de <span className="font-bold text-slate-700">{ultimaPagina}</span> 
            <span className="hidden sm:inline"> (Total: {totalRegistros} registos)</span>
          </p>
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="flex justify-between items-center p-6 border-b border-slate-100">
              ...
              <button onClick={fecharGaveta} className="p-2 bg-slate-50 text-slate-400 hover:text-slate-600 rounded-full transition-colors"><X size={20} /></button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <form id="form-cliente" onSubmit={handleSalvarCliente} className="space-y-5">
                <div>
                  ...
                  <input type="text" required className="w-full border border-slate-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-sky-500" value={novoCliente.nome} onChange={e => setNovoCliente({...novoCliente, nome: e.target.value})} />
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">E-mail</label>
                    <input type="email" className="w-full border border-slate-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-sky-500" value={novoCliente.email} onChange={e => setNovoCliente({...novoCliente, email: e.target.value})} />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Telefone</label>
                    <input type="text" className="w-full border border-slate-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-sky-500" value={novoCliente.telefone} onChange={e => setNovoCliente({...novoCliente, telefone: e.target.value})} />
                  </div>
                </div>

                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">CPF / CNPJ</label>
                  <input type="text" required className="w-full border border-slate-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-sky-500" value={novoCliente.cpf_cnpj} onChange={e => setNovoCliente({...novoCliente, cpf_cnpj: e.target.value})} />
                </div>

                <div className="bg-slate-50 p-5 rounded-xl border border-slate-200 mt-6 relative">
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-2">
                      ...
                      <div className="w-2/3">
                        ...
                        <input type="text" readOnly className="w-full border border-slate-200 rounded-lg p-2 text-sm bg-slate-100 text-slate-500" value={novoCliente.cidade} />
                      </div>
                    </div>
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Logradouro / Bairro</label>
                      <input type="text" readOnly className="w-full border border-slate-200 rounded-lg p-2 text-sm bg-slate-100 text-slate-500" value={novoCliente.endereco ? `${novoCliente.endereco}, ${novoCliente.bairro}` : ''} />
                    </div>
                  </div>
                </div>
              </form>
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              <button type="button" onClick={fecharGaveta} className="px-5 py-2.5 text-slate-600 font-bold hover:bg-slate-200 rounded-lg transition-colors">Cancelar</button>
              <button type="submit" form="form-cliente" className="bg-sky-600 text-white px-6 py-2.5 rounded-lg font-bold hover:bg-sky-700 transition-colors shadow-md">
            ...
            <div className="p-6 text-center">
              ...
              <div className="flex gap-3">
                ...
                <button 
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-red-600 text-white font-bold hover:bg-red-700 transition-colors shadow-sm shadow-red-200"
                >
                  Sim, excluir
                </button>
              </div>
